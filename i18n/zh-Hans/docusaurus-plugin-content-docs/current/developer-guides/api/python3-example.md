...
> 本页面提供了一个 Python3 示例客户端，用于通过 MikroTik RouterOS 的 API 与其进行交互，演示了使用 socket 通信和 MD5 哈希进行认证的登录及命令执行过程。

# Python3 示例

一个简单的 Python3 示例客户端。

- 用法：`api.py <ip-address> <username> <password> <secure(True|False)>`，
  例如：`api.py 10.0.0.1 admin Badpassword123 True`
- 之后，从键盘输入单词，并以换行符结尾
- 由于空单词会终止一个句子，因此您应在最后一个单词后按 **两次** 回车键，以将句子发送至路由器。
