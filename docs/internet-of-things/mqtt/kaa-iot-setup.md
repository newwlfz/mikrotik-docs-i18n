...
[MQTT](./index.md) and [HTTP](../../system-information-and-utilities/fetch.md) are among the most popular protocols that are used for transferring all kinds of data. Both protocols are heavily utilized in different IoT setups, and they both are supported by [RouterOS](../../getting-started/software-specifications/index.md).

What kind of data, you might ask? Pretty much anything... RouterOS [scripting](../../developer-guides/scripting/index.md) is a very powerful tool that can help you automate your devices with the help of [scheduling](../../system-information-and-utilities/scheduler.md).
...
This command shows you useful information, like CPU usage, RAM-memory usage, device's uptime and its firmware version. Another command will print out [GPS](../../mobile-networking/gps/index.md) coordinates (if the device has a GPS chip built-in) and so on...

What this means, essentially, is that anything that can be "printed out" into the RouterOS [terminal](../../management-tools/console.md), can be scripted to be structured into JSON format messages and sent out with a configured interval. We will showcase a more detailed example later on in the guide.

In other words, you are able to send the data from your MikroTik to any MQTT or HTTP server of your choice. [Kaa IoT](https://www.kaaiot.com/) is one such server.

[Why Kaa IoT?](https://www.kaaiot.com/advantages/platform)

## Kaa IoT configuration

Make sure to check [KAA: Connecting your first device](https://www.kaaiot.com/docs/tutorials/getting-started/connecting-your-first-device).

After you log into the portal, to start, follow the steps from the [playbook](https://www.kaaiot.com/docs/tutorials/getting-started/connecting-your-first-device):
...
First things first, the device should have internet access. Check our [First Time Configuration](../../getting-started/first-time-configuration.md) guide.

Once you have the Internet up and running, if you are planning to use [MQTT](./index.md) protocol, make sure you have the **iot** package installed. You can download it from our [download page](https://mikrotik.com/download), under the "Extra packages" file (for your device's respective architecture). Unzip the "Extra packages" file and upload the **iot** package to the "[Files](../../system-information-and-utilities/files.md)" (reboot the device after that). If you are planning on only using [HTTP posting](../../system-information-and-utilities/fetch.md), there is no need for the iot package installation.

Before going further, visit Kaa IoT [MQTT](https://www.kaaiot.com/docs/device-connectivity/mqtt-device-api) and [HTTP](https://www.kaaiot.com/docs/device-connectivity/http-device-api) device API guides.
...
, where you should change `<app-version>` (that you can check under the "**Home>Device management>Devices>Specific device**" tab) and `<token>` (that you've generated after creating a device on the platform) to your respective values.
...
In order to use a one-way SSL MQTT scenario, get the root certificate from "**Home>Device management>Credentials**" by clicking on the "**Get root certificate**" button. More info can be found [here](https://www.kaaiot.com/docs/tutorials/getting-started/authenticating-client-with-tls-certificate).
