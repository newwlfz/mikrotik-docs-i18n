...
> This page documents MQTT integration in MikroTik RouterOS, covering publisher/subscriber workflows for IoT platforms like Kaa and ThingsBoard. It explains MQTT protocol basics, RouterOS capabilities as an MQTT broker/client, and provides configuration examples for AWS/Azure cloud platforms.
...
RouterOS can act as an MQTT publisher and subscriber (starting with **7.11beta2**). You can also run an MQTT broker/server via the [container](../../containers/index.md) feature. For Mosquitto MQTT broker configuration visit the [link here](../../containers/user-guides/container-mosquitto-mqtt-server.md).
...
c) [MQTT and ThingsBoard configuration](./mqtt-and-thingsboard-configuration.md)
...
| **certificate** (*string*; Default: ) | [Certificate from certificate store](../../authentication-authorization-accounting/certificates.md) that is going to be used for the SSL connection. |
...
| **password** (*string*; Default: ) *[sensitive](../../getting-started/configuration-management/list-of-menus-with-sensitive-parameters.md)* | Password for the broker (if required by the broker). |
...
This section is used to manage already-added subscriptions (that were previously added via the [Subscribe](./index.md#subscribe) section).

It has the same properties as the [Subscribe](./index.md#subscribe) section.
...
| Property | Description |
| :-- | :-- |
| **on-message** (*string*; Default: ) | Configure a [script](../../developer-guides/scripting/index.md) that will be automatically initiated/run whenever a new message is received in the subscribed topic. |
...
To run a [script](../../developer-guides/scripting/index.md) (for example, a basic "log" script) whenever any new message appears in the subscribed topic, you can use the `on-message` feature:
...
- The same applies to [global](../../developer-guides/scripting/index.md#scopes) variable usage. If there are global variables that are "generated" using other scripts (variables that appear under the System>Script>Environment section), they will not work inside the "on-message" script.
...
A second example shows how to run a script whenever a specific message (keywords from the message) appears. To achieve a scenario where we want to run a script only when the MQTT message has specific content or a keyword, we can utilize the **if** [condition statement](../../developer-guides/scripting/index.md#conditional-statement):
...
You can also use [scripts](../../developer-guides/scripting/index.md) to structure MQTT messages that contain RouterOS statistics. Then, you can apply the [scheduler](../../system-information-and-utilities/scheduler.md) to run the script whenever you like.

For example, you can run a script like [mqttpublish.rsc](pathname:///assets/200573055_mqttpublish.rsc) (copy the content of the RouterOS code shown below into a new terminal and press <kbd>Enter</kbd>):
