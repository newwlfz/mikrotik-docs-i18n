...
| **check-certificate** | *yes* &#124; *no* | *yes* | Verifies the registry certificate against the router’s [certificate store](../../authentication-authorization-accounting/certificates.md) before pulling the container image. |
...
:::warning
Port `80` is assumed as the web port by default, unless explicitly named (e.g., `888:80:web`). The healthcheck probes this port — if nothing is listening, the app will appear stuck in a `starting` state. When port `80` is not mapped, the first defined port is used instead. To avoid ambiguity, name your used ports explicitly:
