...
> MikroTik Traffic-Flow provides network monitoring and accounting by collecting statistical information about packets passing through the router, supporting NetFlow versions 1, 5, 9, and IPFIX for flexible flow analysis. It allows configuration of interfaces, cache entries, timeouts, packet sampling, and target hosts for flow data transmission.
...
By looking at the [packet flow diagram](../../firewall-and-quality-of-service/packet-flow-in-routeros.md) you can see that traffic flow is at the end of the input, forward, and output chain stack. It means that traffic flow will count only traffic that reaches one of those chains.
