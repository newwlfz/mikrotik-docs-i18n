...
> The Profiler tool in RouterOS displays CPU usage for each process, helping identify resource-intensive processes. It supports per-core monitoring and classifies CPU usage by process type for efficient debugging.
...
Watch our [video about this feature](https://youtu.be/BkRaW14p8_s).
...
The "CPU" parameter allows specifying an integer number which represents a core or one of two predefined values **all** and **total:**

- total - this value is set to show the sum of all core usages.
- all - value is set to show CPU usages separately for every available core.
