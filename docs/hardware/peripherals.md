...
RouterOS v7 supports the following cellular modems:

- MikroTik modems.
- Third-party modems supported by device class/type:
  - MBIM class USB interface
  - USB-CDC class USB interface
  - RNDIS type USB interface
- Third-party modems with added support (see the modem table below).

## Please note

- Not all modems are listed in the supported modem table. Some modems may work even if not listed because modem manufacturers often re-use the same hardware IDs, and vice versa.
- Customized, localized, or locked units may have compatibility issues.
- Third-party modems may require configuration adjustments before they can be used with RouterOS.
- For mini-PCIe modems with a USB3.0 interface installed in a mini-PCIe slot with PCIe/USB2.0 support, the USB speed must be limited to USB2.0, or the mini-PCIe shared PCIe/USB3.0 pin must be isolated. See the picture below the table.

## Cellular modems

| Model | vendor-id | device-id | Tested RouterOS version | Comments | Format |
| :-- | :-- | :-- | :-- | :-- | :-- |
| Alcatel IK40 |  |  | v6.41RC11 | LTE interface, Modem can be configured only through modem's configuration WEB page. | USB |
...
| Cinterion LTE Modem | 0x1e2d | 0x0061 | v7.14 and higher | LTE interface, some settings are ignored. Not full SMS functionality. | MiniPCI-e |
...
| D-link DWM-222 |  |  | See comments  | Multiple modem versions with the same marketing name exist: H/W ver: A1 - supported as config-less LTE interface from RoS v6.38Modem firmware version starting from "2.1.4EU".H/W ver: A3 - works as config-less LTE using modem firmware provided RNDIS interface (RouterOS v7 only) | USB |
...
| Dell Wireless 5530 HSPA |  |  | v6.1 and higher | Data channel 0, Info channel 0, init: AT+CFUN=1 (needs to manually change profile by command AT\*ENAP=1,1) | MiniPCI-e |
...
| Huawei E3372h, E5576h, E8372h | 0x12d1 | 0x14db | v6.8 | Config-less LTE interface for modems with vendor-id="0x12d1" device-id="0x14db"  Models with suffixes -320 and -608 will not work with RouterOS v6, please use v7 instead  Models with suffixes -325 work only with arm cpu | USB |
| Huawei E3276-150 |  |  | v6.xx | ppp interface | USB |
| Huawei E3351 |  |  | v6.24 and higher |  | USB |
| Huawei E3531 |  |  | v6.24 or 6.40RC25 | There are different versions of this modem E3531-6 works from version 6.40RC25 as ppp, mbim is supported only from RouterOS V7 | USB |
...
| Huawei ME909s-120 |  |  | v6.28 | Recommended modem firmware version 11.617.24.00.00. To reduce LTE interface IP subnet mask to /32 configure modem with at-chat command: `/interface/lte/at-chat [find]` input="AT^CUSTFEATURE=3,1" | MiniPCI-e |
...
| Quectel EC20/EC21 |  |  | v6.xx | ppp interface | MiniPCI-e |
| Quectel BG77 | 0x2c7c | 0x0700 | v6.47 | Serial/PPP interface, single AT/modem channel | OEM module |
| Quectel BG95-M3 |  |  | v6.47 | Serial/PPP interface, single AT/modem channel  Will not work on wAP R ac boards. | mini-PCIe |
| Quectel BG96 |  |  | v6.45 | Serial/PPP interface, 2x AT/modem channels | mini-PCIe |
| Quectel EC25-EU | 0x2c7c | 0x0125 | v6.42 | ppp/LTE interface.  RouterOS v6  CDC-ECM mode - LTE interface receive address in modem's internal network.  RouterOS v7 MBIM mode -  LTE interface uses APN IP address. | MiniPCI-e |
| Quectel EG25-G | 0x2c7c | 0x0125 | 6.48.3 | RouterOS v6  CDC-ECM mode - LTE interface receive address in modems internal network.  RouterOS v7 MBIM mode -  LTE interface uses APN IP address.  On some boards it may be required to disable SIM hot plug detection: ```/interface/lte/set [find] modem-init="AT+QSIMDET=0,1"``` | MiniPCI-e |
| Quectel EM12-G | 0x2c7c | 0x0512 | v7.1beta5 | MBIM driver | m.2 |
| Quectel EP06 |  |  | v6.42 | ppp/LTE interface | MiniPCI-e |
| Quectel RG255C-GL |  |  | v7.18 | Basic support with at-chat, minimum firmware revision: RG255CGLABR01A04M4G\_A0.004.A0.004  Modem composition: `at+qcfg="usbnet",0` | mini-PCIe |
...
| Sierra wireless MC73xx |  |  | v6.xx(ppp) v7.xx (LTE) | Works! PPP interface. And starting with v7.xx it will support LTE interface with the modem switched to expose the MBIM interface. MC7304 tested with firmware SWI9X15C\_05.05.67.00 | MiniPCI-e |
...
| Sierra Wireless MC7710/MC7700/MC7750 |  |  | v5.25, v6.0 and 6.40RC43 | If the modem uses firmware 3.5 it should be upgraded to 3.5.23.2 firmware release in order to work in RouterOS correctly again. | MiniPCI-e |
...
| Vodafone (Huawei) K4203 |  |  | v7.xx | Not supported in ROS v6, but as this modem supports MBIM driver support will be possible in ROS v7. | USB |
...
| ZTE MF823 |  |  | v6.8 | Some settings are ignored. For some devices it's needed to enter FACTORY mode to change operating state. | USB |
...
| ZTE MF833U1 | 0x19d2 | 0x1405 | v7.20.6 | Config-less LTE interface | USB |
| ZTE MF90 |  |  | v6.44beta32 and higher | LTE interface | USB |

:::warning
Not all modems are listed. Localized and locked units may have compatibility issues.
...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| MikroTik | S-85DLC05D | 1,25G | Dual LC, MM | 850nm | \*Check:  [SFP/SFP+ compatibility reference table](../wired-connections/mikrotik-wired-interface-compatibility.mdx) | Natively supported |
| MikroTik | S-31DLC20D | 1,25G | Dual LC, SM | 1310nm | \*Check:  [SFP/SFP+ compatibility reference table](../wired-connections/mikrotik-wired-interface-compatibility.mdx) | Natively supported |
| MikroTik | S-35LC20D | 1,25G | BiDi LC, SM | Tx:1310nm/Rx:1550nm | \*Check:  [SFP/SFP+ compatibility reference table](../wired-connections/mikrotik-wired-interface-compatibility.mdx) | Natively supported |
| MikroTik | S-53LC20D | 1,25G | BiDi LC, SM | Tx:1550nm/Rx:1310nm | \*Check:  [SFP/SFP+ compatibility reference table](../wired-connections/mikrotik-wired-interface-compatibility.mdx) | Natively supported |
| MikroTik | S-RJ01 | 1000/100/10 | RJ45, Cat5/Cat6 | N/A | \*Check:  [SFP/SFP+ compatibility reference table](../wired-connections/mikrotik-wired-interface-compatibility.mdx) | Natively supported |
...
| Dell (Finisar) | FTLX8571D3BCL | 300m | 10G | Dual LC, MM | 850nm | Most of the SFP/SFP+ MikroTik products | Works! |
| Juniper (Finisar) | FTLX8571D3BCL-J1 | 300m | 10G | Dual LC, MM | 850nm | Most of the SFP/SFP+ MikroTik products | Works! |
| Intel (Finisar) | FTLX8571D3BCV-IT | 300m | 10G | Dual LC, MM | 850nm | Most of the SFP/SFP+ MikroTik products | Works! |
| OEM (Juniper?) | EX-SFP-10GE-SR-OEM | 300m | 10G | Dual LC, MM | 850nm | Most of the SFP/SFP+ MikroTik products | Works! |
...
The RDS2216-2XG-4S+4XS-2XQ supports U.2 SSD drives of any capacity, with a physical height limitation of 7mm. While any drive meeting this physical requirement should be supported, the following models has been verified:
