...
- usage: `api.py <ip-address> <username> <password> <secure(True|False)>`,
  for example: `api.py 10.0.0.1 admin Badpassword123 True`
- after that, type words from the keyboard, terminating them with a new line
- Because an empty word terminates a sentence, you should press enter **twice** after the last word to send a sentence to the router.
...
def main():
	...
	if arg_nr > 4: secure = sys.argv[4].lower() == 'true'
