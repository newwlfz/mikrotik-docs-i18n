...
It is possible to create a centralized Access Point management setup for a home or office environment that is scalable to many Access Points. Such a setup is quite easy to configure and has been explained in the [Simple CAPsMAN setup](./ap-controller-capsman.md#simple-setup-of-a-capsman-system) guide, but for more complex setups VLANs might be required. CAPsMAN has the functionality to assign a certain VLAN ID under certain conditions. This guide will provide an example on how to assign a VLAN ID to Wireless packets based on the AP, to which a wireless client connects. CAPsMAN with VLANs can be achieved either by using [Local Forwarding Mode](./ap-controller-capsman.md#local-forwarding-mode) or [CAPsMAN Forwarding Mode](./ap-controller-capsman.md#manager-forwarding-mode), the Local Forwarding Mode will provide the possibility to use a switch between your APs and CAPsMAN router to switch packets (to achieve larger throughput), while CAPsMAN Forwarding Mode should be used when all traffic should always be forwarded to the CAPsMAN router (in most cases to filter packets).

In this example, we are going to assign all our Wireless clients to **VLAN10**, if they connect to **WiFi\_WORK**, and we are going to assign Wireless clients to **VLAN20**, if they connect to **WiFi\_GUEST**. We are going to use Virtual APs along with CAPsMAN to create multiple SSIDs for our Wireless clients to connect to while using a single physical device. An example of how to use a single SSID for a single physical device will also be shown by using CAPsMAN provisioning rules.
...
- We are going to create a single CAPsMAN provisioning rule to create the **WiFi\_WORK** and the **WiFi\_GUEST** SSIDs on a single device, and each connected CAP is going to create these SSIDs automatically.
...
In this example, we are going to be using [Bridge VLAN Filtering](../../../bridging-and-switching/index.md#vlan-example-trunk-and-access-ports) to filter unknown VLANs and to assign other devices to the same networks. Some devices are capable of offloading this to the built-in switch chip, check the [Basic VLAN switching](../../../bridging-and-switching/user-guides/basic-vlan-switching.md) guide to see how to configure it on different types of devices.
...
- Create a bridge and assign a port to it, which is connected to the CAPsMAN Router:
...
- After CAPs are successfully connected to the CAPsMAN Router, the wlan1 (SSID **WiFi\_WORK**) and a newly created virtual wlan5 (SSID **WiFi\_GUEST**) interfaces get dynamically added as bridge ports. The VLAN is assigned for a wireless interface and as a result, all data coming from wireless gets tagged and only data with this tag will be sent out over wireless. Bridge vlan-filtering can be disabled if additional VLAN managing and controlling is not needed. The associated VLAN can be seen with the port VLAN ID (PVID) property:
...
:::info
 CAPsMAN will attach CAP interfaces to the bridge and will automatically add appropriate entries to the bridge VLAN table.
:::
...
- After CAPs are successfully connected to the CAPsMAN Router, two CAP interfaces will be dynamically created on the CAPsMAN Router. Both of these interfaces will get dynamically added as bridge ports on the same CAPsMAN Router due to explicitly selecting the bridge interface with datapath.bridge=bridge1 and using the default CAPsMAN Forwarding Mode datapath.local-forwarding=no. Because of using a bridge with vlan-filtering enabled, both CAP interfaces will also show up in a bridge VLAN table:
...
Not everyone wants to create Virtual APs since that does decrease the total throughput. If you want to use multiple devices to create multiple SSIDs, then it is possible to assign a certain configuration to a CAP based on its identity. To achieve this you should use CAPsMAN provisioning rules along with RegEx expressions. In this example we are going to assign the **Config\_WORK** configuration to CAPs that have an identity set to "'**AP\_WORK\_\***" and we are going to assign the **Config\_GUEST** configuration to CAPs that have an identity set to "**AP\_GUEST\_\***". To do this, you simply need to change the CAPsMAN provisioning rules.
...
- Create new provisioning rules that will assign appropriate configuration to a CAP based on its identity:
