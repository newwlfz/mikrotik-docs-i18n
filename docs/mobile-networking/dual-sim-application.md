...
The first script example shows how to switch between SIM slots in case mobile roaming is detected for LtAP mini devices. This could be useful for mobile vehicle applications, where cars, buses or trains could drive abroad and should use two SIM cards (one for a home network, the other for a roaming network). Since RouterOS has a roaming status in the LTE monitor (displayed only when roaming), we can use this in RouterOS scripts to change SIM cards accordingly.
...
First, make sure you have correctly set up LTE network parameters (provided by the mobile network operator) for each SIM card. You can use the default APN profile or create two separate ones, follow this link -  [LTE/5G#Quicksetupexample](./lte-5g.md#quick-setup-example). This example uses the default APN profile.
...
Then, choose which SIM slots will be used for home and roaming networks. In this example, we use slot "down" for home and slot "up" for roaming network. Some device models like SXT have SIM slots named "a" and "b" instead of "up" and "down". Use the following command to switch between active slots.

### Command in RouterOS v6 version
...
After changing SIM slots, the LTE modem will be restarted. It can take some time (depending on the modem and board around 30 seconds) to fully initialize it, so make sure you test your modem.
...
:if ($isLTEinit=true) do={
    # Wait for LTE interface to connect to mobile network for maximum "timeoutConnect" seconds
...
Lastly, create your scheduler that will run the previously created script. Choose a proper scheduler interval, so two or more events do not overlap with each other. For this example above, 3 minutes will be enough.
