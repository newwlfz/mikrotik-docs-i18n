...
> The MAC server section in RouterOS enables configuring MAC Telnet, WinBox, and Ping servers for router access without IP addresses. It allows restricting access to specific interfaces and provides tools to manage sessions, with examples showing how to enable/disable MAC Telnet and connect via client.
...
It is possible to set MAC Telnet access to specific interfaces that are a part of the [interface list](../getting-started/configuration-management/list-of-menus-with-sensitive-parameters.md):
...
Same as with MAC Telnet, it is possible to set MAC WinBox access to specific interfaces that are a part of the [interface list](../getting-started/configuration-management/list-of-menus-with-sensitive-parameters.md):
